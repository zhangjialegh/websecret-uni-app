<template>
	<view class="game-body">
		...
		<view class="above-game">
			...
			<text class="restart-button" @tap="restart">新游戏</text>
		</view>

		<view class="game-container">

			<!--游戏结束显示  -->
			<view class="game-message" :class="{'game-over': over && !win, 'game-won': over && win}">
				<text class="over-msg">{{overMsg}}</text>
				<view class="lower">
					<!-- <text class="keep-playing-button">继续</text> -->
					<text class="retry-button" @tap="restart">再试一次</text>
				</view>
			</view>

			<view class="grid-container" @touchstart="touchStart" @touchmove="touchMove" @touchend="touchEnd">
				<view class="grid-row" v-for="(row, rowIdx) in grids" :key="rowIdx">
					<view class="grid-cell" v-for="(cell, colIdx) in row" :key="colIdx">
						<view v-if="cell" class="tile" :class="['tile'+cell.value]">
...
<script>
	...
	let isMultiple = false // 多手指操作
	const thisGameManager = new GameManager(4)
	...
	export default {
		...
		onLoad() {
			this.grids = thisGameManager.setup()
		...
		methods: {
			...
			touchStart: function(events) {
				// 多指操作
				isMultiple = events.touches.length > 1;
				if (isMultiple) {
					return;
				}
			...
			touchEnd: function(events) {
				...
				if (Math.max(absDx, absDy) > 10) {
					...
					var data = thisGameManager.move(direction) || {
						grids: this.grids,
						over: this.over,
						won: this.won,
						score: this.score
					...
					var highscore = uni.getStorageSync('highscore') || 0;
					if (data.score > highscore) {
						uni.setStorageSync('highscore', data.score);
					}
